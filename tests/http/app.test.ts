...
import { AuthFailTracker, keyMatches } from "../../src/http/auth.js";
import { COMMAND_MAX_BYTES } from "../../src/http/consoleCommand.js";
import { WRAPPER_VERSION } from "../../src/contracts/version.js";
...
describe("POST validation", () => {
  ...
  it("rejects an over-long command (A-08)", async () => {
    const res = await app.inject({
      ...
      url: "/instances/survival/command",
      headers: auth,
      payload: { command: "x".repeat(COMMAND_MAX_BYTES + 1) },
    });
    expect(res.statusCode).toBe(400);
  });

  // The cap is in bytes, not characters: a multi-byte command shorter than
  // COMMAND_MAX_BYTES in .length can still overflow the RCON payload.
  it("measures the command cap in bytes, not UTF-16 units", async () => {
    const res = await app.inject({
      method: "POST",
      url: "/instances/survival/command",
      headers: auth,
      payload: { command: "ä".repeat(COMMAND_MAX_BYTES / 2 + 1) },
    });
    expect(res.statusCode).toBe(400);
  });

  // Length was never the control that mattered here: a newline reaching a
  // console sink is a second command.
